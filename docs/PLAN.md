...
| M1 | **Walking skeleton + CI** | `go.mod`, `cmd/server`, config, slog, chi, `/healthz` `/readyz`, graceful shutdown, Docker Compose (PostGIS), goose (embedded migrations), Makefile (`make check`), golangci-lint (+ module-boundary rules), air; **CI**: `ci.yml` (tidy · lint · tests `-race` with PostGIS · build), PR-title check, Dependabot, branch ruleset | packages & modules, `main` wiring, `context`, `http.Server`, errors as values; workflow anatomy, service containers, caching, required checks | Add `GET /version` using `debug.ReadBuildInfo` |
| M1.5 | **CD & releases** | Dockerfile (multi-stage, distroless nonroot), `cd.yml` (release-please → multi-arch build once → Trivy scan → SBOM + provenance attestations → GHCR `:sha` → promote on release), `security.yml` (govulncheck, gitleaks, dependency review, CodeQL), compose runs a released image | artifacts vs. source, immutable tags, SemVer + Conventional Commits, supply-chain security, build once / promote | Cut `v0.1.0` and run it locally from GHCR; verify its attestation |
| M2 | **Shared kernel + IAM** (3 PRs: shared kernel → OTP login → tokens & `/me`) | `api/openapi.yaml` + oapi-codegen, sqlc, generated-code drift check in CI; value objects (PhoneNumber, Money, LocalizedText, GeoPoint), OTP request/verify (console SMS adapter), Ed25519 JWT, refresh rotation, auth middleware, `GET /me` | constructors & invariants, unexported fields, consumer-side interfaces, sqlc + pgx transactions, table-driven tests, a database per test | Implement `PATCH /me` |
